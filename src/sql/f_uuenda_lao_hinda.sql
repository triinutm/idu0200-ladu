CREATE OR REPLACE FUNCTION f_uuenda_lao_hinda (item.item%TYPE, item_action.item_count%TYPE, item_action.action_price%TYPE) 
RETURNS void AS $$
...
toodte_laohind_ennem_lisamist = (SELECT coalesce(store_price,0) FROM item where item = $1);
